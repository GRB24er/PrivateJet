...
(async () => {
  try {
    ...
    // 1) Ensure client user exists with a REAL password field
    ...
    if (!client) {
      ...
      // Ensure password + role are correct in case schema changed
    ...
    // 2) Get jets to attach bookings to
    ...
    // 3) Realistic travel history for this client
    const sampleBookings = [
      {
        ...
        origin: "KTEB",
        destination: "KBOS",
        // IMPORTANT: use `departureAt` / `arrivalAt` to match your Booking model
    ...
    // 4) Wipe ONLY this client's old bookings
    await Booking.deleteMany({ client: client._id });

    const created = await Booking.insertMany(sampleBookings);
    console.log(
      `✅ Seeded ${created.length} bookings for client (${client.email})`
    );

    console.log(`
👤 Client login (REAL account, not demo label):
  Email:    ${email}
  Password: Client#123

Use this on /login to see Dashboard + My Trips with real travel history.
    `);
